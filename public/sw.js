const CACHE_NAME = "saudi-weather-v3";
const PRECACHE_URLS = [
  "/",
  "/index.html",
  "/offline.html",
    ...
    event.respondWith(
      fetch(request)
        ...
        .catch(async () => {
          ...
          return new Response(
            '<!doctype html><html><head><meta charset="utf-8"><title>Offline</title></head><body><h1>Offline</h1><p>Content is unavailable while offline.</p></body></html>',
...
self.addEventListener("push", (event) => {
  ...
  event.waitUntil(
    self.registration.showNotification(data.title || "Saudi Weather", options),
  );
});
